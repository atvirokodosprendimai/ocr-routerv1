...
| `internal/store/repo_test.go` | edit | The failing tests below |
...
```bash
set -o pipefail
go test ./internal/store/... -run 'TestSetRateCarriesRawMode|TestServiceModeDefaultsToUnits' -count=1 -v 2>&1 | tee /tmp/acc-0006-T1.out \
...
|-----------|------|----------|--------|-------|
| `TestSetRateCarriesRawMode` | `internal/store/repo_test.go` | `SetRate(label, n, true)` then `ServiceMode(label)` reports raw; setting it back to false clears it | — | S2, S4 |
| `TestServiceModeDefaultsToUnits` | `internal/store/repo_test.go` | A label with no `service_rates` row, and a row inserted without the column, both read as units-mode — the migration's default is what an existing deployment gets | — | S2, S4 |
| `TestCreateJobPersistsRawStamp` | `internal/store/repo_test.go` | A job created with `Raw: true` reads back `Raw: true`; the default is false | — | S3 |
| `TestMigrationDownDropsRawColumns` | `internal/store/repo_test.go` | `goose down` to 00002 leaves a schema the pre-ADR code opens — Rollback step 1 is executable, not asserted | — | S2 |
