...
| T3 | The login page, the cookie, and a CSRF check that fails closed | done | T1, T2 | `go test ./internal/web/... -race` |
| T4 | Set passwords from the CLI, sweep sessions on the existing tick | done | T1, T2, T3 | `go test ./cmd/router/... -race` |
