...
| `TestOnlyLoginAndHealthzAreUnauthenticated` | `cmd/router/monitoring_test.go` | no route serves a 2xx to a caller with no credential, except an explicit allow-list — asserted alongside the exception so making `/healthz` public cannot quietly make a sibling public. ⚠ **Renamed and rewritten by ADR-0003**, which added the two login routes. It was originally `TestHealthzIsTheOnlyUnauthenticatedRoute` and probed a hand-written list of six paths; that list went stale the moment a route was added, and the replacement walks the real chi route table instead. The invariant it guards is unchanged — *only deliberate exemptions answer unauthenticated* — but the permitted set grew from one to three, which is ADR-0003's decision and is recorded there | — | S3 |
